...
首次启动后，右键状态栏图标 → **Settings...** 打开设置窗口，填入 Mininglamp 的 Base URL 和 API Key，点击 Save 即可。配置持久化在 `~/Library/Application Support/com.brding.vibe-usage-monitor/config.json`。

| 配置项 | 说明 |
|--------|------|
| Base URL | Mininglamp API 基础地址（如 `https://api.example.com`） |
| API Key | Mininglamp API 密钥 |

`VIBE_MONITOR_RUNTIME_DIR` 环境变量可指定运行时目录（默认当前工作目录，打包后为 app data dir）。
...
- `config.json`：来源凭证配置（通过 Settings UI 管理）
